#!/bin/bash
#SBATCH --job-name=continuum_production_v1.1
#SBATCH --nodes=1
#SBATCH --ntasks=1
...
cd "$MCFOST_RUN_DIR"
: "${MCFOST_UTILS:?Export MCFOST_UTILS before configuring/submitting}"
...
exec /SET/WORKING/PYTHON -B code/workflow.py task "$MCFOST_RUN_DIR" --index "$SLURM_ARRAY_TASK_ID" --machine "$MCFOST_MACHINE"

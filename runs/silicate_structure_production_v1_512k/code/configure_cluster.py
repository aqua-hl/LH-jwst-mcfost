...
def is_production(experiment):
    return experiment.get("experiment_id") in {PRODUCTION_EXPERIMENT, SILICATE_PILOT, SILICATE_STRUCTURE}
...
def bundle_layout(bundle):
    ...
    if is_production(experiment):
        ...
        if experiment["experiment_id"] == SILICATE_STRUCTURE:
            if len(tasks) != 60 or not all((bundle / relative).is_file() for relative in
                    ("code/analyze_silicate_structure_production.py", "code/check_materials.py")):
                raise ValueError("Silicate structure campaign requires 60 tasks, its analyzer and material preflight")
        return experiment
...
def launch_scripts(bundle, experiment, machine, configured):
    ...
    structure = experiment.get("experiment_id") == SILICATE_STRUCTURE
    analyzer = ("analyze_silicate_structure_production.py" if structure
                else "analyze_silicate_size_pilot.py" if experiment.get("experiment_id") == SILICATE_PILOT
    ...
    submit = ["#!/bin/bash", "set -euo pipefail",
              ...
              *(['sbatch --parsable --export=ALL job_materials.sh > "$submission_dir/materials_sbatch.out"',
                 'materials_job=$(<"$submission_dir/materials_sbatch.out"); materials_job=${materials_job%%;*}',
                 '[[ "$materials_job" =~ ^[0-9]+$ ]] || { echo "Unexpected material sbatch output; inspect $submission_dir" >&2; exit 1; }',
                 'printf "%s\\n" "$materials_job" > "$submission_dir/materials_job_id.txt"',
                 'echo "Submitted material initialization check $materials_job; array starts only if it passes."',
                 'sbatch --parsable --export=ALL --dependency="afterok:$materials_job" --kill-on-invalid-dep=yes job_array.sh > "$submission_dir/array_sbatch.out"']
                if structure else ['sbatch --parsable --export=ALL job_array.sh > "$submission_dir/array_sbatch.out"']),
    ...
    scripts = {"job_array.sh": array, "job_analysis.sh": analysis, "submit.sh": submit}
    if structure:
        scripts["job_materials.sh"] = common + ["#SBATCH --cpus-per-task=2", "#SBATCH --time=00:15:00",
            "#SBATCH --mem=8G", "#SBATCH --output=logs/materials-%j.out", "#SBATCH --error=logs/materials-%j.err"] + body + [
            f'exec {python} -B code/check_materials.py "$MCFOST_BUNDLE_DIR" --machine "$MCFOST_MACHINE"']
    return scripts

...
def expand_models(config):
    ...
    for i, override in enumerate(overrides):
        ...
        parameters = {key: value if key == "envelope_silicate_file" else float(value)
...
def prepare_run(config_path):
    ...
    selected_optical_names = {m["parameters"]["envelope_silicate_file"] for m in models
                              if "envelope_silicate_file" in m["parameters"]}
    if selected_optical_names - dust_sources.keys():
        raise ValueError(f"Models require supplied dust_files: {sorted(selected_optical_names - dust_sources.keys())}")

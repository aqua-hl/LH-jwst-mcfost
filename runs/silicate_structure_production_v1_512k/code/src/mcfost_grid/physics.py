...
PHYSICAL_KEYS = frozenset({
    ...
    "envelope_density_exponent", "envelope_silicate_file",
})
ENVELOPE_SILICATE_FILES = frozenset({
    "Draine_Si_sUV.dat", "Olivine_MgFeSiO4_Dorschner1995_mcfost.dat",
    "Pyroxene_Mg05Fe05SiO3_Dorschner1995_mcfost.dat",
})
...
def validate_parameters(parameters: Mapping[str, object]) -> None:
    ...
    for name, value in parameters.items():
        if name == "envelope_silicate_file":
            if not isinstance(value, str) or value not in ENVELOPE_SILICATE_FILES:
                raise ValueError("envelope_silicate_file must name one of the three supported, run-local silicate tables")
            continue
        number = _number(value, name)
        if name == "envelope_density_exponent":
            if not -3 < number < 0:
                raise ValueError("envelope_density_exponent must lie strictly between -3 and 0")
        elif name in {"inclination_deg", "accretion_rate_msun_per_year", "envelope_size_exponent"}:
            if number < 0:
...
def _disjoint_dhs_scopes(t: _Template, envelope: range) -> tuple[range, range]:
    ...
    weights can be edited by this deliberately bounded renderer. The envelope
    silicate may use one of three explicitly supported material filenames;
    re-rendering retains the selected table while the disk stays unchanged.
    """
    ...
    for scope, filename in zip(scopes, ("Draine_Si_sUV.dat", "H2O_30K_Leiden_mcfost.dat")):
        ...
        allowed = ENVELOPE_SILICATE_FILES if filename == "Draine_Si_sUV.dat" else {filename}
        if optical[0] not in allowed or float(optical[1]) != 1:
...
def render_parameter(template_text: str, parameters: Mapping[str, object],
    ...
    ``envelope_silicate_file`` is limited to three known tables in that DHS
    envelope only. ``envelope_density_exponent`` sets its volume-density power
    law; any mass normalization is an explicit caller responsibility.
    """
    validate_parameters(parameters)
    if not isinstance(numerics, Mapping):
        raise ValueError("numerics must be a mapping")
    ...
    if disjoint_dhs:
        ...
        if "envelope_silicate_file" in parameters:
            raise ValueError("Explicit envelope_silicate_file is supported only for the disjoint DHS envelope")
        if envelope_count != ["1"]:
    ...
    if "envelope_density_exponent" in parameters:
        density_tag = "surface density exponent"
        exponents = t.values(density_tag, 2, envelope_density)
        exponents[0] = float(parameters["envelope_density_exponent"])
        t.set(density_tag, exponents, envelope_density)
    ...
    if disjoint_dhs:
        if "envelope_silicate_file" in parameters:
            t.set("Optical indices file", [parameters["envelope_silicate_file"], 1.0], silicate_grains)
